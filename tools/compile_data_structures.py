...
def emit_c(
    ...
    anm_max = gun_icon_anm_ext_max(custom_count + cannon_count)
    ...
    lines.append(
        ...
        f"+{custom_count * 2} impact+{cannon_count * 2} cannon frames). */"
    )
...
def main() -> int:
    ...
    for path in (enemy_path, impact_path, cannon_path):
        if not path.is_file():

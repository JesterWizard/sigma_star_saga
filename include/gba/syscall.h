...
#define CpuSet(src, dest, control)                                                          \
    do                                                                                      \
    {                                                                                       \
        _Static_assert(_Alignof(src) >= __builtin_choose_expr(__builtin_constant_p(control), \
            ((control) & CPU_SET_32BIT) ? 4 : 2, 2), "source potentially unaligned");       \
        _Static_assert(_Alignof(dest) >= __builtin_choose_expr(__builtin_constant_p(control), \
            ((control) & CPU_SET_32BIT) ? 4 : 2, 2), "destination potentially unaligned");  \
